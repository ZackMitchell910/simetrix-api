    ...
    for row in rows:
        ...
        parsed.append(
            FeedRecord(
                ...
                asof=asof,
                source=str(row.get("source", "macro")),
...
async def fetch(
    ...
    *,
    ...
    """Fetch macroeconomic context relevant to the asset."""

    symbol_upper = symbol.upper().strip()
    rows: list[Mapping[str, Any]] = []
    if store is not None:
        key = f"macro:{symbol_upper}:{asof.date().isoformat()}:{window}"
    ...
    if not rows:
        rows.append(
            {
                "symbol": symbol_upper,
                "asof": (asof - timedelta(days=1)).isoformat(),
                "source": "macro",
                "confidence": 0.2,
                "payload": {"rates_delta_bp": 0.0, "inflation_yoy": 0.0},
                "tags": ("synthetic",),
            }
        )
