    ...
    for row in rows:
        ...
        parsed.append(
            FeedRecord(
                ...
                asof=asof,
                source=str(row.get("source", "social")),
...
async def fetch(
    ...
    *,
    ...
    """Fetch aggregated social/alt sentiment metrics."""

    symbol_upper = symbol.upper().strip()
    rows: list[Mapping[str, Any]] = []
    if store is not None:
        key = f"social:{symbol_upper}:{asof.date().isoformat()}:{window}"
    ...
    if not rows:
        rows.append(
            {
                "symbol": symbol_upper,
                "asof": (asof - timedelta(minutes=45)).isoformat(),
                "source": "social",
                "confidence": 0.15,
                "payload": {"last24h": 0.0, "last7d": 0.0},
                "tags": ("synthetic",),
            }
        )
