...
async def require_key(
    ...
    if provided:
        ...
        if REDIS:
            plan = await get_plan_for_key(REDIS, provided)
        else:
            plan = PLAN_DEFAULT
...
class Settings(BaseSettings):
    ...
    macro_source: Optional[str] = Field(
        default=None,
        description="Macro data provider identifier (e.g., 'fred').",
    )
    earnings_source: Optional[str] = Field(
        default=None,
        description="Earnings data provider identifier (e.g., 'polygon').",
    )
    ...
    sim_queue_concurrency: int = Field(
        default_factory=lambda: int(os.getenv("PT_SIM_QUEUE_CONCURRENCY", "0")),
        ge=0,
        le=16,
        description="If >0, run simulations through a shared dispatcher with limited workers.",
    )
...
def _has_admin_access(request: Request) -> bool:
    ...
    return any(scope in {"admin", "cron"} for scope in scopes)

async def _ensure_run_owned(run_id: str, request: Request) -> "RunState":
    rs = await _ensure_run(run_id)
    owner = rs.owner
    caller = getattr(request.state, "caller_id", None)
    if owner and owner != caller and not _has_admin_access(request):
...
if "get_ensemble_prob_light" not in globals():
    async def get_ensemble_prob_light(symbol: str, redis: "Redis", horizon_days: int = 1) -> float:
        try:
            ...
            feature_vector = [float(f.get(feat, 0.0)) for feat in feats]
            X = np.array(feature_vector, dtype=float)

            if model_linear.get("onnx_path"):
                prob = _run_linear_onnx(symbol, feature_vector)
                if prob is not None:
                    return prob
...
async def _run_daily_quant(horizon_days: int | None = None) -> dict:
    ...
    def _safe_earnings() -> dict:
        return {"surprise_last": 0.0, "guidance_delta": 0.0, "days_since_earn": None, "days_to_next": None}

    async def _load_context(sym: str) -> dict:
        key = (sym or "").upper().strip()
        if not key:
            return {"sentiment": _safe_sentiments(), "earnings": _safe_earnings()}
        ...
        try:
            ...
            earn = _safe_earnings()
        ctx = {"sentiment": sent or _safe_sentiments(), "earnings": earn or _safe_earnings()}
        context_cache[key] = ctx
        return ctx

    def _mu_bias_from_context(ctx: dict) -> float:
        sent = ctx.get("sentiment") or {}
        earn = ctx.get("earnings") or {}
        return float(
            np.clip(
                float(sent.get("avg_sent_7d") or 0.0) * 0.15
                + float(earn.get("surprise_last") or 0.0) * 0.05,
                -0.15,
                0.15,
            )
        )

    async def _enrich_candidate(item: dict) -> dict:
        sym = str(item.get("symbol") or "").upper()
        ctx = await _load_context(sym)
        sent = ctx.get("sentiment") or _safe_sentiments()
        earn = ctx.get("earnings") or _safe_earnings()
        mu_bias = _mu_bias_from_context(ctx)
    ...
    async def _enrich_candidate_list(cands: list[dict]) -> list[dict]:
        if not cands:
            return []
        tasks = [asyncio.create_task(_enrich_candidate(dict(item))) for item in cands]
        enriched = [await t for t in tasks]
        enriched.sort(
            key=lambda r: (float(r.get("score") or 0.0), float(r.get("score_raw") or 0.0)),
            reverse=True,
        )
        return enriched

    async def _enrich_results_list(items: list[dict]) -> list[dict]:
        if not items:
            return []
        enriched: list[dict] = []
        for item in items:
            ...
            result["context"].update(
                {
                    "sentiment": sent,
                    "earnings": ctx.get("earnings") or _safe_earnings(),
                    "macro": macro_ctx,
                    "mu_bias": mu_bias,
                }
            )
            if "prob_up_end" in result and result.get("prob_up_end") is not None:
                ...
                prob_adj = float(
                    np.clip(
                        prob_raw
                        + 0.35 * mu_bias
                        + 0.10 * float(sent.get("last24h") or 0.0),
                        0.0,
                        1.0,
                    )
                )
            ...
            if "median_return_pct" in result and result.get("median_return_pct") is not None:
                ...
                med_adj = float(med_raw + mu_bias * 100.0 * 0.2)
    ...
    if eq_precandidates:
        ...
        eq_seed: list[str] = await llm_shortlist("equity", list(eq_watch), top_k=seed_k, horizon_days=horizon)
    ...
    if cr_precandidates:
        ...
        cr_seed: list[str] = await llm_shortlist("crypto", list(cr_watch), top_k=seed_k, horizon_days=horizon)
        cr_rank = await _rank_candidates(cr_seed, top_k=rank_k)

    eq_rank = await _enrich_candidate_list(eq_rank)
    cr_rank = await _enrich_candidate_list(cr_rank)
    ...
    eq_mc_ok = await _enrich_results_list(eq_mc_ok)
    cr_mc_ok = await _enrich_results_list(cr_mc_ok)
...
async def _trigger_daily_quant_from_health(reason: str = "health") -> None:
    """
    Kick off the daily quant run when the service wakes up (e.g., via health probes).
    Ensures we only spawn one background task per day and respects Redis markers.
    """
    global _LAST_SCHEDULED_QUANT, _HEALTH_QUANT_TASK, _HEALTH_QUANT_LAST

    today_str = datetime.now(timezone.utc).date().isoformat()
    if _LAST_SCHEDULED_QUANT == today_str:
        return

    async def _has_done_flag() -> bool:
        if not REDIS:
            return False
        try:
    ...
    # Fast path: honor prior Redis marker before taking the lock
    if await _has_done_flag():
    ...
    async with _HEALTH_QUANT_LOCK:
        ...
        task = asyncio.create_task(_runner(), name="quant_health_trigger")
...
async def compare_backtest(
    ...
    for symbol in req.symbols:
        ...
        for start_idx in range(0, upper - lookback):
            ...
            rets = _winsorize(rets)
            sigma_bar = _ewma_sigma(rets, lam=0.94)
            sigma_ann_raw = float(sigma_bar * math.sqrt(scale))
            sigma_ann = float(np.clip(sigma_ann_raw, 1e-4, SIGMA_CAP))
            ...
            shrink = _horizon_shrink(horizon)
            mu_ann *= shrink
